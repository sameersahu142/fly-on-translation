...
const SideSheetContent = ({ handelClose, handelExpand }) => {
  ...
  const translate = (selectLanguage, selectText) => {
    setIsLoading(true);
    translateYourSelectedText(selectLanguage, selectText)
      .then((response) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%" }}>
      <div className="headingController">
        <div className="heading">{translation.translateValue}</div>
        <ExpandAndCrossIcon
          handelCloseBtn={handelClose}
          handelExpandBtn={handelExpandBtn}
        />
      </div>
      <div className="selectLanguageContainer">
        ...
        >
          <option key={-1} value="">
            {translation.selectLanguageValue}
          </option>
      ...
      <div
        className={
          isExpand ? "translateTextExpandContainer" : "translateTextContainer"
        }
      >
        {isLoading ? (
          <div
            className={isExpand ? "ExpandLoadingContainer" : "loadingContainer"}
          >
            <FontAwesomeIcon icon={faSpinner} className="fa-pulse" size="2xl" />
          </div>
        ) : (
          <div
            className={
              translatedText.length > 0 ? "translateAreaAuto" : "translateArea"
            }
          >
            {translatedText}
          </div>
